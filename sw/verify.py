...
AUDIO_FILENAME = 'recovered_audio.wav'
...
output_file = os.path.join(output_path, 'outputs.txt')
expected_out_file = os.path.join(PROJECT_DIR, 'firmware', 'expected_output.txt')
...
assert len(out_cplx) == len(expected_cplx), (
    f"Output length does not match expected length: {len(out_cplx)} != {len(expected_cplx)}. "
    "The generated firmware/input assets are likely out of sync with this simulation run."
)
