...
def build_project_description_checks(
    ...
    core_ok = (
        ...
        and abs(core_w - BASELINE["core_w_um"]) <= 1e-6
        and abs(core_h - BASELINE["core_h_um"]) <= 1e-6
    )
    rows.append(
        (
    ...
    pwr_rpt_ok = (root / "pnr/finalReports/report_power.rpt").exists()
    rows.append(("Post-layout power estimation report", "PASS" if pwr_rpt_ok else "FAIL", "pnr/finalReports/report_power.rpt"))
    ...
    # VCD start/duration window check from script
    vcd_cmd = read_text(root / "sim_struct/scripts/run_vcd.cmd")
    vcd = parse_vcd_window_from_cmd(vcd_cmd)
    has_window = all(vcd.get(k) for k in ["start_val", "start_unit", "dur_val", "dur_unit"]) and vcd.get("has_vcd_on_off") == "yes"
    rows.append(
        (
            "VCD start+duration configured",
            "PASS" if has_window else "FAIL",
            f"start={fmt(vcd.get('start_val'))}{fmt(vcd.get('start_unit'))}, duration={fmt(vcd.get('dur_val'))}{fmt(vcd.get('dur_unit'))}, on/off={fmt(vcd.get('has_vcd_on_off'))}",
        )
    )

    # VCD window consistency with behavioral run numbers
    beh_start_ms = parse_float(sim_behav.get("first_chunk_start_ms"))
    beh_dur_us = parse_float(sim_behav.get("first_chunk_us"))
    vcd_start_ms = to_ms(vcd.get("start_val"), vcd.get("start_unit"))
    vcd_dur_us = to_us(vcd.get("dur_val"), vcd.get("dur_unit"))
    start_match = (
        beh_start_ms is not None and vcd_start_ms is not None and abs(beh_start_ms - vcd_start_ms) <= 1e-3
    )
    dur_match = beh_dur_us is not None and vcd_dur_us is not None and abs(beh_dur_us - vcd_dur_us) <= 1e-3
    win_match = start_match and dur_match
    rows.append(
        (
            "VCD window matches behavioral start/runtime",
            "PASS" if win_match else "WARN",
            f"beh_start={fmt(sim_behav.get('first_chunk_start_ms'))}ms vs cmd_start={fmt(None if vcd_start_ms is None else f'{vcd_start_ms:.6f}')}ms; beh_dur={fmt(sim_behav.get('first_chunk_us'))}us vs cmd_dur={fmt(None if vcd_dur_us is None else f'{vcd_dur_us:.6f}')}us",
        )
    )
...
def build_markdown(root: Path) -> str:
    ...
    final_power_text = read_text(root / "pnr/finalReports/report_power.rpt")
    innovus_log = read_text(root / "pnr/innovus.log")
    ...
    chip_total_mw = parse_float(power_map.get("report_power.rpt", {}).get("total"))
    accel_energy = None
    chip_energy = None
    ...
    comparison_rows = compare_vs_baseline_rows(
        setup_wns=parse_float(timing_map.get("final_setup", {}).get("wns_all")),
        hold_wns=parse_float(timing_map.get("final_hold", {}).get("wns_all")),
        accel_total_power=parse_float(power_hier.get("accel_total")),
        chip_total_power=parse_float(power_map.get("report_power.rpt", {}).get("total")),
        accel_leakage=parse_float(power_hier.get("accel_leakage")),
        latency_us=chunk_latency_us,
        accel_energy_nj=accel_energy,
    ...
    comparison_rows_md = [
        ...
        for metric, current_v, baseline_v, delta_v, status_v in comparison_rows
    ]
...
- whole-chip total power: **{fmt(power_map.get('report_power.rpt', {}).get('total'))} mW**
- clock network power (`clk`): **{fmt(power_hier.get('clk_total'))} mW**
- clock period used in report: **{fmt(power_map.get('report_power.rpt', {}).get('clock_period_usec'))} usec**
- clock toggle rate used in report: **{fmt(power_map.get('report_power.rpt', {}).get('clock_toggle_mhz'))} MHz**
...
|---|---:|---:|---:|---|
{chr(10).join(comparison_rows_md)}
