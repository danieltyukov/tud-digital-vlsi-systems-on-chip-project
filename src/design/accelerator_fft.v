...
module accelerator_fft #(
  ...
  localparam SCALE = 12;
  localparam MUL_W = 2 * MEM_WIDTH + 1;

  // Make Q12 arithmetic width/signedness explicit so RTL and synthesis agree.
  function signed [MEM_WIDTH-1:0] q12_mul;
    input signed [MEM_WIDTH-1:0] a;
    input signed [MEM_WIDTH-1:0] b;
    reg signed [MUL_W-1:0] prod;
    begin
  ...
  function signed [MEM_WIDTH-1:0] q12_cmul_re;
    input signed [MEM_WIDTH-1:0] ar;
    input signed [MEM_WIDTH-1:0] ai;
    input signed [MEM_WIDTH-1:0] wr;
    input signed [MEM_WIDTH-1:0] wi;
    reg signed [MUL_W-1:0] prod_rr;
    reg signed [MUL_W-1:0] prod_ii;
    reg signed [MUL_W-1:0] diff;
    begin
      prod_rr = $signed(ar) * $signed(wr);
      prod_ii = $signed(ai) * $signed(wi);
      diff = prod_rr - prod_ii;
      q12_cmul_re = diff >>> SCALE;
    end
  endfunction

  function signed [MEM_WIDTH-1:0] q12_cmul_im;
    input signed [MEM_WIDTH-1:0] ar;
    input signed [MEM_WIDTH-1:0] ai;
    input signed [MEM_WIDTH-1:0] wr;
    input signed [MEM_WIDTH-1:0] wi;
    reg signed [MUL_W-1:0] prod_ri;
    reg signed [MUL_W-1:0] prod_ir;
    reg signed [MUL_W-1:0] sum;
    begin
      prod_ri = $signed(ar) * $signed(wi);
      prod_ir = $signed(ai) * $signed(wr);
      sum = prod_ri + prod_ir;
      q12_cmul_im = sum >>> SCALE;
    end
  endfunction
  ...
  assign recomb_prod_re = q12_cmul_re(temp2_re, temp2_im, w32_re, w32_im);
  assign recomb_prod_im = q12_cmul_im(temp2_re, temp2_im, w32_re, w32_im);
        ...
        S_COMPUTE: begin
          ...
          case (compute_step)
            ...
            4'd2: begin  // q1 = data[5] * W_16^1 = (3784, -1567)
              temp1_re <= q12_cmul_re(data_re[5], data_im[5], 32'sd3784, -32'sd1567);
              temp1_im <= q12_cmul_im(data_re[5], data_im[5], 32'sd3784, -32'sd1567);
            end
            4'd3: begin  // q2 = data[9] * W_16^2 = (2896, -2896)
              temp2_re <= q12_cmul_re(data_re[9], data_im[9], 32'sd2896, -32'sd2896);
              temp2_im <= q12_cmul_im(data_re[9], data_im[9], 32'sd2896, -32'sd2896);
            end
            4'd4: begin  // q3 = data[13] * W_16^3 = (1567, -3784)
              temp3_re <= q12_cmul_re(data_re[13], data_im[13], 32'sd1567, -32'sd3784);
              temp3_im <= q12_cmul_im(data_re[13], data_im[13], 32'sd1567, -32'sd3784);
            end
            ...
            4'd6: begin  // q1 = data[6] * W_16^2 = (2896, -2896)
              temp1_re <= q12_cmul_re(data_re[6], data_im[6], 32'sd2896, -32'sd2896);
              temp1_im <= q12_cmul_im(data_re[6], data_im[6], 32'sd2896, -32'sd2896);
            end
            ...
            4'd8: begin  // q3 = data[14] * W_16^6 = (-2896, -2897)
              temp3_re <= q12_cmul_re(data_re[14], data_im[14], -32'sd2896, -32'sd2897);
              temp3_im <= q12_cmul_im(data_re[14], data_im[14], -32'sd2896, -32'sd2897);
            end
            ...
            4'd10: begin  // q1 = data[7] * W_16^3 = (1567, -3784)
              temp1_re <= q12_cmul_re(data_re[7], data_im[7], 32'sd1567, -32'sd3784);
              temp1_im <= q12_cmul_im(data_re[7], data_im[7], 32'sd1567, -32'sd3784);
            end
            4'd11: begin  // q2 = data[11] * W_16^6 = (-2896, -2897)
              temp2_re <= q12_cmul_re(data_re[11], data_im[11], -32'sd2896, -32'sd2897);
              temp2_im <= q12_cmul_im(data_re[11], data_im[11], -32'sd2896, -32'sd2897);
            end
            4'd12: begin  // q3 = data[15] * W_16^9 = (-3784, +1569)
              temp3_re <= q12_cmul_re(data_re[15], data_im[15], -32'sd3784, 32'sd1569);
              temp3_im <= q12_cmul_im(data_re[15], data_im[15], -32'sd3784, 32'sd1569);
            end
