module accelerator_fft #(
  ...
  wire [2*MEM_WIDTH-1:0] w_m_pair;
  wire signed [MEM_WIDTH-1:0] w_m_re_const;
  wire signed [MEM_WIDTH-1:0] w_m_im_const;

  // Twiddle step per stage (Q12), used to remove twiddle-memory reads.
  function automatic [2*MEM_WIDTH-1:0] stage_twiddle_step;
    input [LOG_MAX_FFT_STAGES-1:0] stage_idx;
    begin
      case (stage_idx)
        5'd1: stage_twiddle_step = {-32'sd4096, 32'sd0};         // exp(-j*2pi/2)
        5'd2: stage_twiddle_step = {32'sd0, -32'sd4096};         // exp(-j*2pi/4)
        5'd3: stage_twiddle_step = {32'sd2896, -32'sd2896};      // exp(-j*2pi/8)
        5'd4: stage_twiddle_step = {32'sd3784, -32'sd1567};      // exp(-j*2pi/16)
        5'd5: stage_twiddle_step = {32'sd4017, -32'sd799};       // exp(-j*2pi/32)
        default: stage_twiddle_step = {32'sd4096, 32'sd0};
      endcase
    end
  endfunction

  assign w_m_pair = stage_twiddle_step(stage);
  assign w_m_re_const = w_m_pair[2*MEM_WIDTH-1:MEM_WIDTH];
  assign w_m_im_const = w_m_pair[MEM_WIDTH-1:0];
  ...
  always @(posedge clk) begin
    if (reset_accel) begin  // Reset registers
      ...
      case (state_reg)
        ...
        READ_W_M_IM: ;  // No twiddle-factor memory read in this variant
  ...
  always @(*) begin
    ...
    case (state_reg)
      ...
      READ_W_M_RE: ;  // Twiddle comes from local stage constants
      READ_W_M_IM: ;  // Twiddle comes from local stage constants
      BUTTERFLY_READ_1_RE: begin
