/*##########################################################################
###
### EE v4: D1 register-file + hardcoded twiddle LUT + trivial-twiddle fast paths
###
###     Based on EE v3 (D1 register-file with hardcoded combinational
###     twiddle LUT).  Adds trivial-twiddle detection and fast paths:
###     when w is one of {+1, -1, +j, -j}, the full complex multiply is
###     bypassed and replaced by sign flips / real-imaginary swaps.
###
###     Trivial-twiddle coverage for N=32 (80 butterflies total):
###       Stage 1 (m=2,  16 bf): k=0 w=+1       -> ALL 16 trivial
###       Stage 2 (m=4,   8 bf): k=0 w=+1, k=1 w=-j -> ALL 8 trivial
###       Stage 3 (m=8,  16 bf): k=0 w=+1, k=2 w=-j -> 8 of 16 trivial
###       Stage 4 (m=16, 16 bf): k=0 w=+1, k=4 w=-j -> 4 of 16 trivial
###       Stage 5 (m=32, 16 bf): k=0 w=+1           -> 1 of 16 trivial
###                                                     (k=8 is (-4,-4095)
###                                                      due to rounding,
###                                                      so NOT matched)
###       Total: 37 of 80 butterflies (~46%) skip the full multiplier.
###
###     FSM phases:
...
module accelerator_fft #(
  ...
  /*========================================================================================
        TRIVIAL-TWIDDLE DETECTION
        When w is exactly {+1, -1, +j, -j} in Q12, bypass the full complex
        multiply and use sign flips / real-imaginary swaps instead.
        Exact Q12 values:  +1 = (4096, 0),  -1 = (-4096, 0),
                           +j = (0, 4096),   -j = (0, -4096)
    ========================================================================================*/
  wire twiddle_is_pos_one;
  wire twiddle_is_neg_one;
  wire twiddle_is_pos_j;
  wire twiddle_is_neg_j;
  wire twiddle_is_trivial;

  assign twiddle_is_pos_one = (lut_w_re ==  32'sd4096) && (lut_w_im == 32'sd0);
  assign twiddle_is_neg_one = (lut_w_re == -32'sd4096) && (lut_w_im == 32'sd0);
  assign twiddle_is_pos_j   = (lut_w_re == 32'sd0)     && (lut_w_im == 32'sd4096);
  assign twiddle_is_neg_j   = (lut_w_re == 32'sd0)     && (lut_w_im == -32'sd4096);
  assign twiddle_is_trivial = twiddle_is_pos_one | twiddle_is_neg_one
                            | twiddle_is_pos_j   | twiddle_is_neg_j;
  ...
  always @(*) begin
    // --- Twiddle x v multiplication with trivial-twiddle fast paths ---
    if (twiddle_is_pos_one) begin
      // w = +1: t = v (pass through, no multiply)
      t_re = data_re[idx_v];
      t_im = data_im[idx_v];
    end else if (twiddle_is_neg_one) begin
      // w = -1: t = -v (negate both components)
      t_re = -data_re[idx_v];
      t_im = -data_im[idx_v];
    end else if (twiddle_is_pos_j) begin
      // w = +j: t_re = -v_im, t_im = +v_re
      t_re = -data_im[idx_v];
      t_im =  data_re[idx_v];
    end else if (twiddle_is_neg_j) begin
      // w = -j: t_re = +v_im, t_im = -v_re
      t_re =  data_im[idx_v];
      t_im = -data_re[idx_v];
    end else begin
      // Full complex multiply (non-trivial twiddle)
      t_re = (data_re[idx_v] * lut_w_re - data_im[idx_v] * lut_w_im) >>> SCALE;
      t_im = (data_re[idx_v] * lut_w_im + data_im[idx_v] * lut_w_re) >>> SCALE;
    end

    // --- Butterfly add / subtract (unchanged) ---
  ...
  always @(posedge clk) begin
    if (reset_accel) begin
      ...
      case (state_reg)
        ...
        //  Trivial-twiddle fast paths bypass full complex multiply
        // ==============================================================
        S_COMPUTE: begin
